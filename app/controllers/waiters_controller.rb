class WaitersController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @waiter.update(waiter_params)
        format.html { redirect_to menucard_path, notice: 'Waiter was successfully updated.' }
        format.json { render :show, status: :ok, location: menucard_path }
      else
  ...
  def destroy
    @waiter.destroy
    respond_to do |format|
      format.html { redirect_to menucard_path, notice: 'Waiter was successfully destroyed.' }
  ...
  private
    ...
    def waiter_params
      params.require(:waiter).permit(:name, :salary)
    end
end
